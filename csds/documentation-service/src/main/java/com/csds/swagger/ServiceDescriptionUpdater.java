...
public class ServiceDescriptionUpdater {
	...
	private static final Map<String, String> notToDocumentedMapping = new HashMap<>();
	...
	public void refreshSwaggerConfigurations() {
		...
		discoveryClient.getServices().stream().forEach(serviceId -> {
			...
			if (serviceInstances == null || serviceInstances.isEmpty()) { // Should not be the case kept for failsafe
				...
				String instanceName = instance.getServiceId();
				String contextPath = notToDocumentedMapping.get(instanceName);
				if (contextPath == null) {
	...
	private String getSwaggerURL(ServiceInstance instance) {
		String instanceName = instance.getServiceId();
		String contextPath = urlMapping.get(instanceName);
		contextPath = contextPath == null ? "" : contextPath;
		String swaggerURL = instance.getMetadata().get(KEY_SWAGGER_URL);
		return swaggerURL != null ? instance.getUri() + contextPath + swaggerURL
				: instance.getUri() + contextPath + DEFAULT_SWAGGER_URL;
	}
	...
	private Optional<Object> getSwaggerDefinitionForAPI(String serviceName, String url) {
		...
		try {
			...
			logger.error("Error while getting service definition for service : {} Error : {} , URL : {} ", serviceName,
					ex.getMessage(), url);
