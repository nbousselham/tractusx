...
public interface DataspaceConnectorRulesApi {
	...
	RuleResponse registerRule(URI baseUrl, @RequestHeader("Authorization") String authHeader,
			@RequestBody RuleDescription metadata);

	@DeleteMapping(path = "/rules/{id}")
	void deleteRule(URI baseUrl, @RequestHeader("Authorization") String authHeader, @PathVariable("id") UUID id);
}
