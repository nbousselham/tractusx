...
public interface ShellRepository extends PagingAndSortingRepository<Shell, UUID> {
    ...
    @Query("select distinct s.id_external from shell s where s.id in (select distinct si.fk_shell_id from shell_identifier si where CONCAT(si.domain, ':', si.identifier) in (:keyValueCombinations))")
